...
namespace
{
                          ...
                          double& score)
  {
    ...
    if (dmp > connect_dpad_slope)
    {
      return false;
    }
    if (dmt > connect_dtbin_slope)
    {
      return false;
    }

    score = (dp / connect_dp) * (dp / connect_dp) + (dt / connect_dt) * (dt / connect_dt) + (dmp / connect_dpad_slope) * (dmp / connect_dpad_slope) + (dmt / connect_dtbin_slope) * (dmt / connect_dtbin_slope) + 0.05 * static_cast<double>(gap);
  ...
  struct RawHitTimeSort
  {
    ...
    explicit RawHitTimeSort(const std::vector<InModuleThreadData::RawHit>* h)
      : raw_hits(h)
    {
    }
    bool operator()(unsigned int a, unsigned int b) const
  ...
  void reject_long_pad_noise(InModuleThreadData* d)
  {
    ...
    if (d->noise_max_consecutive_timebins <= 0)
    {
      return;
    }
    ...
    for (auto& it : by_layer_pad)
    {
      ...
      if (idx.size() <= static_cast<unsigned int>(d->noise_max_consecutive_timebins))
      {
        continue;
      }
                              ...
                              double pred_tbin)
  {
    ...
    for (unsigned int i = 0; i < d->blobs.size(); ++i)
    {
      ...
      if (std::fabs(dp) > d->search_dp)
      {
        continue;
      }
      if (std::fabs(dt) > d->search_dt)
      {
        continue;
      }

      const double score = (dp * dp) / (d->search_dp * d->search_dp + 1.0e-9) + (dt * dt) / (d->search_dt * d->search_dt + 1.0e-9) - 0.01 * std::log(bl.adc + 1.0);
      if (score < best_score)
      {
        best_score = score;
                          ...
                          int direction)
  {
    while (true)
    {
      ...
      if (direction < 0)
      {
        edge_layer = d->blobs[chain.front()].layer;
      }

      if (direction > 0 && edge_layer >= 54)
      {
        break;
      }
      if (direction < 0 && edge_layer <= 7)
      {
        break;
      }
  ...
  for (const auto& td : tdata)
  {
    for (unsigned int it = 0; it < td.tracks.size(); ++it)
    {
      ...
      const unsigned int global_track_id =
          m_tpcModuleTrackContainer ? m_tpcModuleTrackContainer->size() : static_cast<unsigned int>(m_tree_track_id.size());
